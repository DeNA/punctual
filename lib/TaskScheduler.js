 ...
 */
TaskScheduler.create = function(opts){
    if( this instanceof TaskScheduler ) return this;
    else return new TaskScheduler(opts);
};
...
TaskScheduler.prototype._setupRedisSubscription = function(cb){
    ...
    this.redisClient.once('ready', onRedisReady);
    this._stopSteps.push(function() {
        // remove listeners upon stop
 ...
 * @param {Object} [opts] - optional params hash
 * @param {Function} cb({Error}, {Array} Tasks)
 */
 ...
 * @param {Function} [cb({Error}, runtimeResponses)]
 */
 ...
 * @param {Function} [cb] - passed thru to TaskRunner.init method
 */
TaskScheduler.prototype.registerTaskRunner = function(runner, taskTypes, runnerName, cb){
    ...
    if(!runner || !_und.isFunction(runner.runTask)){
        var err = new Errors.InvalidTaskRunner("'runTask' is not a function" + util.inspect(runner));
    ...
    if( _und.isFunction( runner.init ) ) return runner.init(cb);
    else return runner;
};
 ...
 * @param {Function|String} runnerInst - see TaskRunner class
 * @return {String[]|Bool} taskRunners - array of updated taskRunners
 */
TaskScheduler.prototype.deregisterTaskRunner = function(runnerInst){
    ...
    if(!this.taskRunners[runnerName]){
        var err = new Errors.InvalidTaskRunner("No TaskRunner found with name " + runnerName);
        this.logger.error(err);
        this.emit('error', err);
        return err;
    }
